<?php
                            ...
                            if($smatiere === "" || $scouleur === "" || $snombre === "" || $ssertis === "") {
                                ...
                                if(!isset($_POST['engagement'])) {
                                    ...
                                    if(isset($_POST['envente']) && ($qt1 === "" || $qt1 === '0')) {
                                        ...
                                        $id_produit = ajouterProduit($conn, $type, $prix, $envente, $titrage, $matierep, $couleurp, $matieres, $couleurs, $motif, $genre, $_SESSION['idVendeur']);
                                        ...
                                        if($qt1 !== "") {
                                            ajouterQuantite($conn, $qt1, $tourdoigt, $poids, $id_produit);
                                        }
                                        header("Location: add_bague.php");
                                        exit();
                                    }
                                }
                            }
    ...
    function ajouterProduit($conn, $type, $prix, $envente, $titrage, $matierep, $couleurp, $matieres, $couleurs, $motif, $genre, $idVendeur) {
        $stmt = $conn->prepare("INSERT INTO produit (type_produit, prix, en_vente, sortie, titrage, 
        matiere_p, couleur_p, matiere_s, couleur_s, motif, id_genre, id_fournisseur) VALUES (:type_produit, :prix, :en_vente, 
        CURDATE(), :titrage, :matiere_p, :couleur_p, :matiere_s, :couleur_s, :motif, :genre, :vendeur)");
    ...
    function ajouterQuantite($conn, $qt1, $tourdoigt, $poids, $id_produit) {
        $stmt = $conn->prepare("INSERT INTO produit_taille (quantite, tour_doigt, poids, id_produit) VALUES
        (:qt1, :tourdoigt, :poids, :id_produit)");
        $stmt->bindParam(':qt1', $qt1);
        $stmt->bindParam(':tourdoigt', $tourdoigt);
